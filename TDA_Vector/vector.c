...
bool vectorCrearDeArchivo(Vector* v, size_t tamElem, const char* nomArch)
{
...
bool vectorGrabar(const Vector* v, const char* nomArch)
{
...
{
    it->primero = v->vec;
    it->ultimo = v->vec + (v->ce - 1) * v->tamElem;
    it->actual = NULL;
    it->finIter = true;
...
{
    if(it->ultimo < it->primero) // Vector vacio.
    {
        return NULL;
    }
