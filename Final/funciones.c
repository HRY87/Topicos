...
int archivoCrearLotes()
{
    ...
    {
        puts("\n----TRABAJOS.CSV---");
        archivoMostrarTxt(PATH_TRABAJOS, accionMostrarLineaTrabajo);
    }
...
void accionMostrarLineaTrabajo(void* dato, void* extra)
{
    ...
    {
        printf("%d\t%d\t%d\t%-6s\n",
               trab.idTrabajo, trab.idModelo,
               trab.cantidad, trab.prioridad);
    }
...
int vectorInsertarOrdenado(tVector* vec, void* dato, size_t tamElem, Comparar comparar)
{
    ...
    if(vec->ce == vec->tope)
        if(vectorRedimenzionar(vec, vec->tope * 2) != TODO_OK)
            return SIN_MEMORIA;

    posIns = (char*)vec->datos;

    if(vec->ce > 0)  /* <-- solo buscar posicion si ya hay elementos */
    {
